...
function TkAudial(a, b) {
  ...
  isNaN(a) || (a = this.roundToStep(a), this.slide(a), this.commitSlide());
...
}, TkAudial.prototype.round = function (a, b) {
  var c = this.decimalPlaces(this.options.step);return this.roundPlaces(a, c);
}, TkAudial.prototype.roundPlaces = function (a, b) {
  var c = Math.pow(10, b);return Math.round(a * c) / c;
}, TkAudial.prototype.roundToStep = function (a) {
  var b = a - this.options.min,
      c = Math.round(b / this.options.step);return this.options.min + this.options.step * c;
  ...
  if (("touchstart" == a.type ? (this.touchCount += a.changedTouches.length, this.active && (this.commitSlide(), this.startY = this.clientY)) : "touchend" == a.type && (this.touchCount -= a.changedTouches.length, this.touchCount < 0 && (this.touchCount = 0), this.active && (this.commitSlide(), this.startY = this.clientY)), a.target.getAttribute("class") && -1 != a.target.getAttribute("class").indexOf("tk-audial"))) {
  ...
  var d = "http://www.w3.org/2000/svg",
      e = "input" == a || "button" == a ? document.createElement(a) : document.createElementNS(d, a);return (e.setAttributeNode(this.attribute("class", b)), "input" == a && (e.setAttributeNode(this.attribute("id", c)), e.setAttributeNode(this.attribute("name", c)), e.setAttributeNode(this.attribute("type", "hidden"))), e);
...
}, TkAudial.prototype.onmousemove = function (b) {
  if (this.startY) {
    this.clientY = b.clientY, b.preventDefault(), b.touches && (b = b.touches[0]);var c = (b.clientY - this.startY) * (b.altKey ? 2 : 1) * (b.shiftKey ? 3 : 1);this.touchCount > 1 && (c = (b.clientY - this.startY) * this.touchCount + 1), (a.altKey || b.shiftKey || this.touchCount > 1 || this.options.alwaysMultiply) && 1 != this.options.sensitivityMultiplier && (c *= this.options.sensitivityMultiplier);var d = this.value + c * this.options.step * -1,
        e = this.round(d);return void this.slide(e);
  }
}, TkAudial.prototype.onmouseup = function (a) {
  if (this.active && (this.commitSlide(), this.startY = null, this.selectable(!0), this.active = !1, this.options.zeroModifiers && (this.touchCount = 0), this.options.enableClipboard && this.clipboarder)) {
    this.clipboarder.setAttributeNode(this.attribute("data-clipboard-text", this.getValue()));var b = new Clipboard(this.clipboarder);this.clipboarder.click(), b.destroy();
  }
